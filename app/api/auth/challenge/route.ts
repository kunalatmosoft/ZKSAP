...
export async function POST(request: NextRequest) {
  try {
    ...
    if (publicKey) {
      ...
      console.log('[v0] Challenge API: Looking up user by Public Key (Recovery)');
      user = db.getUserByPublicKey(publicKey);
    } else {
      // Standard Flow: User entered their username
      console.log('[v0] Challenge API: Looking up user by username:', username);
      user = db.getUserByUsername(username);
    }

    // 3. Handle User Not Found
    if (!user) {
      console.log('[v0] Challenge API: User not found');
      return NextResponse.json(
    ...
    console.log('[v0] Challenge API: Generating challenge for user:', user.id);
    ...
    console.error('[v0] Challenge API error:', errorMessage);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    console.log('[v0] Challenge API: Parsing request');
    const body = await request.json();
    const { username } = body;

    if (!username) {
      console.log('[v0] Challenge API: Missing username');
      return NextResponse.json(
    ...
    console.log('[v0] Challenge API: Looking up user:', username);
    ...
    console.log('[v0] Challenge API: Generating nonce');
    const nonce = generateNonce();

    // Create challenge in database
    const challenge = db.createChallenge(user.id, nonce);
    console.log('[v0] Challenge API: Challenge created:', challenge.id);
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error('[v0] Challenge API error:', errorMessage, error);
    return NextResponse.json(
